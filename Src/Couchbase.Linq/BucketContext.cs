...
namespace Couchbase.Linq
{
    ...
    public class BucketContext : IBucketContext
    {
        ...
        /// <see cref="DocIdAttribute"/> which defines the unique indentifier field for the document.
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <param name="document">The document.</param>
        /// <returns></returns>
        ...
        internal string GetDocumentId<T>(T document)
        {
            ...
            {
                var attribute = (DocIdAttribute)Attribute.GetCustomAttribute(info, typeof(DocIdAttribute));
                if (attribute != null)
                {
                    idName = attribute.DocumentId;
                    if (idName == null)
                    {
                        // ReSharper disable once HeapView.ObjectAllocation
